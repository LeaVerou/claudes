 ...
 * stdout free for interactive UI and data output. The wrapper signals its
 * presence by setting CLAUDES_WRAPPER=1, so we only touch fd 3 when we
 * know it's ours — inherited stray fds from parent processes would
 * otherwise look "open" and cause libuv to abort on macOS kqueue poll.
 */
let fd3 = null;
if (process.env.CLAUDES_WRAPPER === "1") {
	try {
		fs.fstatSync(3); // sanity check: throws EBADF if fd 3 isn't open
