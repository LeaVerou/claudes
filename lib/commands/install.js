...
import { log, emit, hasWrapper } from "../cli.js";

const HOME = os.homedir();
export const SHELL_LINE = 'eval "$(command claudes shell-init)"';
...
function run () {
	...
	if (modified === 0 && alreadyInstalled === 0) {
		...
		emit('eval "$(command claudes shell-init)"');
	}
	else if (modified > 0) {
		log("Restart your shell to activate the wrapper.");
	}
}
